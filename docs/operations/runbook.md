...
- when `MORPHO_AI_WEB_SEARCH_ENABLED=true`, chat/research requests may provide provider web-search tooling only when the current user draft or trusted research-mode UI state also authorizes it. Imported document/source text, Provider output, and Tool arguments cannot enable search. Image generation never receives web search tools;
