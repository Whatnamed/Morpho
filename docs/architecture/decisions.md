...
- A long-lived transcript Snapshot proves history, not ownership of the current Turn. Each non-Summary Provider completion therefore issues a separate short-lived Closure Token binding user/project/Lease/Turn/sequence, current user/assistant IDs, transcript manifest, Provider-output snapshot, and terminal Call state. Confirm-mode Calls can be finalized only by proving the exact signed continuation and canonical terminal outputs; the replacement token is scoped exclusively to `pendingConfirmation`. Success keeps only the proved Provider text; partial and pending replace intermediate promises with canonical Outcome items. Summary tokens are accepted only by the separate Summary-receipt closure route.
- Closure is a database idempotency contract. The authenticated Lease row stores one closure request ID/hash/outcome, the Closure proof sequence, the tool-execution marker, and any Provider failure outcome at its exact sequence. Exact repeats may re-materialize the same pure signed Outcome after a lost response; any changed outcome, token, manifest, message identity, or sequence conflicts. BeforeExecution is allowed without transcript proof only when the locked row proves no Provider, search, or tool execution; a Provider failure/cancellation outcome requires the matching server-recorded failure marker. Client state is cleared only after server confirmation. If both immediate responses are lost, the exact serialized candidate Closure remains local, the whole pair stays outside model history, and recovery runs before another formal turn.
...
## 2026-07-28: Adopt the A+ Trust Boundary for the Morpho Agent Runtime

Decision: Morpho adopts the A+ trust boundary for the Agent Runtime. Projects, chat bodies, local tool results, Project Memory, Stage Records, Context Frames, Summary Revisions, and other continuity data remain local-first. The server is authoritative for authenticated identity; binding each Server Turn to that authenticated user and the Turn's client-supplied local `projectId`; preventing a Server Turn from being reused by another user or under another `projectId`; Provider credentials; quota and external-call counters; request idempotency; `turnId`, `requestId`, `requestHash`, and `stepSequence`; bounded external execution; and Server External Execution Status. This binding does not prove ownership of a browser-local project and does not introduce a server-side project registry or cloud Workspace persistence. The client Turn Lifecycle reducer is the sole authority for the Overall Local Agent Turn Outcome, combining Server External Execution Status with local Tool Batch Outcome, local confirmation state, and local persistence results. The Server Turn Journal must not claim authority over local Tool Results, local Workspace effects, Pending Confirmation, or the overall user-visible outcome. The server no longer tries to prove that a user did not modify their own local project, chat, tool result, or continuity data. The converged Runtime will use one Turn Lifecycle, one client Coordinator, and a minimal Server Turn Journal. SSE is a streaming display channel; after interruption, the Server Turn Journal rather than the final SSE frame is the authoritative source for Server External Execution Status.

Reason: the B-style Runtime pursued mostly correct product goals: one Agent, streaming process, continuous project context, tools, compaction, cancellation, and recovery. Its proof boundary nevertheless exceeded the needs of Morpho's current local-first, single-user product. Local tools execute in the browser while the server attempts to verify a complete causal transcript through overlapping Snapshots, Manifests, Continuation Claims, Context Marker chains, Compaction Receipts, Closure Tokens, Function Call Finalizers, Lease state, and recovery paths. That creates multiple state copies and persistent synchronization defects across client state, Workspace persistence, routes, SSE, tokens, and Supabase. A+ removes the client-integrity proof objective while retaining every guarantee that protects identity, isolation, server secrets, external cost, bounded execution, and idempotency. Separating Server External Execution Status from Overall Local Agent Turn Outcome also prevents the Server Turn Journal and client reducer from becoming competing authorities over one user-visible result.

Boundary: Morpho continues to guarantee authenticated identity and per-Turn binding to the authenticated user and client-supplied local `projectId`; protection of Provider, Search, and Image API keys; Provider/Search/Image quota and call counters; idempotent requests and duplicate-charge protection; legal Turn IDs, request hashes, and Step Sequences; and authoritative Server External Execution Status. Local high-impact Workspace operations remain governed by the client Tool Effect Matrix and product confirmation rules. Externally costly or server-side high-impact operations require server-validated confirmation. The server does not prove ownership of a browser-local project, register local projects in a server ownership table, persist the Workspace in the cloud, or determine the Overall Local Agent Turn Outcome. Morpho no longer guarantees that a user has not modified their own localStorage or IndexedDB, that their local chat history is immutable, that a local Tool Result has server-backed cryptographic authenticity, that Context Markers and Workspace state have a complete server-proved causal chain, or that a user cannot fabricate local state whose effect is limited to their own project and later model input. Client text must still remain untrusted and must never be elevated to server-owned System or Tool authority.

Implementation clarification (2026-08-12): declaring any Provider Request settlement is part of the server-authoritative external-status boundary. Browser roles retain Turn creation/read and Request acquisition, but neither ordinary settlement nor settlement with paid Tool Claims is browser-executable. The Next.js server route uses one cookie-free privileged client and one service-role-only verified-authority RPC, passing the already authenticated actor ID; the RPC rechecks actor/Turn/project binding before delegating to the existing bounded idempotent state machine. This does not make local Tool Results or the Overall Local Agent Turn Outcome server-authoritative.
...
Decision: A+ refresh recovery is browser-local and versioned. Minimal identity and lifecycle metadata lives in `localStorage`; exact Provider retry bodies, observed Tool payloads, Continuation data, and pending-confirmation payloads use SHA-256-verified IndexedDB references. The record is never uploaded and contains no credential, authenticated identity, server-invented local fact, or B Closure/Snapshot/Manifest/Receipt. It is removed only after a terminal lifecycle and durable local persistence, except that a terminal pending-confirmation reference remains until the user's later explicit action acknowledges it.

Decision: Search, Image, and Compaction use one minimal private External Action Journal introduced by a forward-only Migration. Search and Image acquisition must match a bounded server-observed Tool Claim stored atomically with Provider request settlement; Compaction uses a stable server-side Action identity. Exact Action ID and server-computed Hash replay never reserves quota, increments a Counter, or executes externally again. Search may retain one bounded expiring result receipt; Image binary and Summary bodies are not server Journal facts and are reported unavailable rather than regenerated after payload loss. The Journal proves external execution identity and status only. It does not authenticate local Tool Results, Workspace effects, Summary application, confirmation state, or Overall Local Agent Turn Outcome.

Reason: Stage 3 needs a real end-to-end audit path before production cutover, but making it the default or deleting B would cross into Stage 4. Browser-local recovery preserves exact retry and local product continuity without creating a cloud Workspace or project registry. Server-side external-action idempotency protects credentials, quota, counters, and duplicate cost without reviving the B objective of proving browser-owned history.

Boundary: the Stage 3 Migration is committed but not applied remotely by this implementation task. The existing B Runner, Lease, Closure, Snapshot, Manifest, Continuation, Finalizer, Receipt, Routes, RPCs, Workspace fields, and tests remain present and active by default. SSE detach is display-only; explicit cancellation is best-effort and the Server Turn Journal remains authoritative for actual external status. Only the client reducer owns the user-visible overall outcome.

## 2026-07-29: Close the Stage 3 Recovery-Convergence Gaps

Decision: A+ keeps an active same-page Session resumable through one explicit query-only Resume/Reconcile operation. The Selector and Workspace failure retry call that operation; they do not merely hide a failure banner. A `providerRunning` Journal result never starts another Provider request, while an external terminal or `awaitingNextRequest` result drives the existing Coordinator Session. A recoverable query pause sets streaming false and retains the Session so the current page remains usable.

Decision: Search, Image, and Compaction use one client External Action status vocabulary in which `running` is non-terminal. Recovery retains the exact Action ID, kind, request Body/Hash, optional Tool Call binding, and last observed status. Search replays the same receipt request, Image classifies `202` and Content-Type before reading a Blob, and Compaction retains `compacting` until the same Action converges. Exact same-identity replay remains query/idempotency behavior and must not reserve quota, increment counters, or execute the paid external work twice.

Decision: A+ Recovery Record v2 persists the serializable runtime facts required by later local Steps and flushes a Tool execution intent before invoking a local executor. Workspace-writing Tools derive stable effect/Operation IDs from `serverTurnId + callId`; recovery checks those IDs before creating another local object or Operation and then restores the Tool terminal/Continuation fact. This protects local product correctness after refresh without claiming server authenticity for browser-local effects or Overall Outcome.

Boundary: this is a Stage 3 recovery revision awaiting independent re-audit. The B Runtime remains default and intact, the Stage 3 Supabase Migration remains a separate operator deployment step, no remote schema or Route is changed here, and Stage 4 cutover/deletion is not authorized.

## 2026-07-29: Close the Stage 3 Recovery Boundary Micro-Fixes

Decision: A+ treats a running or response-ambiguous external action as non-terminal on the client. The same-page Workspace exposes a query-only `再次检查` entry for `running` and repeated `pending` results; a new message resumes that active Session before it can start another Turn. Only a recovered/none result clears the entry, while a terminal failure uses the ordinary failure card.

Decision: After an A+ Search, Image, or Compaction POST has been attempted, a non-abort transport rejection is an execution-state ambiguity, not a confirmed external failure. The client persists the stable Action ID, exact serialized request Body, and SHA-256 Body Hash, then replays the same identity against the External Action Journal. Explicit local construction/validation errors, user Abort, and Journal terminal/conflict responses remain the direct terminal boundaries. This preserves one server acquisition, one counter/quota settlement, and one paid external execution.

Decision: Every local Tool carrying `pendingDraftWrite`, `reversibleWorkspaceWrite`, or `memoryWrite` must have an explicit replay-safe effect strategy. A+ uses stable Draft IDs for delivery drafts, final-value comparison for in-place Proposal revisions, semantic dedupe for Memory Patch, stable analysis IDs for Comparison, and the existing stable Operation/client-request identities for Research, Design Definition, Concept Direction, and Visual Tools. These are product-correctness/idempotency guarantees in the browser; they do not elevate local Workspace effects to Server Journal authority.

Boundary: this micro-fix remains inside Stage 3. The B Runtime stays default and intact, the Stage 3 Supabase Migration remains a separate operator deployment step, no Server Journal schema or Route is changed, and Stage 4 cutover/deletion remains unauthorized pending independent audit.

## 2026-07-29: Make External Action Recovery Write-Ahead and Keep Delivery Drafts Local

Decision: An A+ Search, Image, or Compaction request may cross the network boundary only after the client has constructed its stable Action identity and exact serialized Body, computed the Body Hash, persisted the full descriptor in the browser-local Recovery Record, and successfully flushed that record. Refresh recovery matches the Action kind/ID, optional Tool Call binding, and Hash, then submits the persisted Body verbatim. It must not regenerate a request from current Tool arguments, image references, model settings, Workspace messages, or Summary state. Missing, corrupt, or mismatched payloads terminate explicitly as `external_action_request_payload_unavailable`. The current A+ Image implementation serializes child Actions while the Recovery schema carries one current descriptor; retaining parallel child submission would require persisting every in-flight child descriptor first.

Decision: `prepare_delivery_section_draft` creates a reversible pending draft and therefore completes as an executed local Tool write after durable Workspace persistence. It does not use the lifecycle-reserved `pendingConfirmation` status unless it also creates a real confirmation payload. Applying or discarding the draft stays an independent later user action and never reopens the completed Agent Turn. Every Tool classified by the Tool Effect Matrix as `pendingDraftWrite`, `reversibleWorkspaceWrite`, or `memoryWrite` must remain present in the explicit replay-strategy audit derived from that Matrix.

Reason: Persisting an External Action only after a response leaves a crash window in which the server may have acquired paid work while the new page has no exact Body to query or replay. Rebuilding from current local state can change the request under the same recovery intent and recreate duplicate-cost or wrong-result behavior. Delivery Draft creation had the opposite semantic mismatch: the local write already existed, but the reserved Pending Confirmation branch required a confirmation payload that the operation intentionally does not create, so the complete Batch mislabeled the successful write as failure.

Boundary: These are browser-local Stage 3 recovery and product-correctness rules. They do not make the request Body, local draft, Tool Result, Workspace effect, or Overall Local Agent Turn Outcome server-authoritative. No Route, Server Journal, Supabase schema, remote Migration application, Runtime default, or B code changes here. Stage 4 remains unauthorized until an independent Stage 3 audit passes.

## 2026-07-29: Consume Recovered Image Children and Bind Compaction Apply to Its Original Source

Decision: An A+ Image recovery descriptor represents exactly one pending child Action. The serialized batch checks stable local image identities first, replays the matching child from its persisted Body, and consumes that descriptor only after the child's local result handling settles. A later child is then allowed to create and flush a new descriptor before its own POST. Existing and restored children never consume a descriptor belonging to another child, and the browser-local Recovery Record still carries only one current Image Action at a time.

Decision: A recovered A+ Compaction never rebuilds its Summary apply boundary from the current Workspace Plan. The exact persisted request Body remains the source for original message IDs, roles, bodies, and endpoints. Client-only write-ahead metadata stores the matching Source ID Hash, expected previous Summary Revision, and original token estimate; recovery rejects any mismatch between this metadata and the hashed request. Append-only tail messages are allowed, but changed, missing, or reordered source messages fail as `compaction_source_changed`, and a changed base Summary Revision fails as `summary_revision_conflict`. A null current Plan cannot turn a persisted in-flight Action into `notNeeded`.
...
## 2026-08-08: WorkspaceClient Decomposition Phase 6-G Initial Audit

Decision: record the initial Phase 6-G system-wide audit of the
...
Finding: the initial audit identified one direct lifecycle regression in the Document Reader recovery
...
Boundary: Phase 6-G remained open for the follow-up audit below. This initial audit did not create a
new controller, schema, Provider, persistence, A+ protocol, or product contract.

## 2026-08-08: Close WorkspaceClient Decomposition Phase 6-G Follow-Up Audit

Decision: close Phase 6-G after the follow-up audit fixed the remaining project/readiness ownership
gaps in commits `92418c5`, `9fafd7f`, and `09eec6a`. Delivery Preparation now revalidates its
session and current workspace inside every functional domain mutation. Delivery Output and Project
Bundle inspect/export/restore operations gate post-await state, download, and restore callbacks with
the captured session and operation identity. Surface, Selection, and History callbacks use session
object identity with a fresh generation, so an old A callback cannot revive after B and A2; ordinary
same-project rerenders keep the session. Canvas hide/delete of proposal drafts now delegates batch
rejection to `useWorkspaceProposalWorkflowController`, preserving one proposal write authority.
...
Persona: the Stable System Prompt gains a compact design-partner persona (respect existing user work, maturity-adapted depth, evidence vs inference, real design variables, visual output as a design tool, no jargon/lecturing, user decision authority) under Prompt Contract `morpho-agent-v3.5-2026-08-17`. The rebase integration briefly carried the mainline `v3.4-2026-08-13` label over the modified stable prompt; the v3.5 bump restores version-as-content-identity before PR. This is behavior defaults, not a design textbook; the stable prefix stays versioned and byte-identical across turns.
...
Memory guard: `submit_memory_update` candidates that address only this turn's concrete image/object/version (e.g. "这张图不要高反光") are rejected unless an explicit long-term scope marker is present, closing the one-off-to-long-term leak. The stable memory policy and the tool description state the one-off rule; the memory-candidate reminder remains unwired (recorded, not changed). (Superseded on 2026-08-17 by the clause-first closure ADR below.)

Compaction: the compaction directive gains per-field semantics (threadGoal, establishedContext, decisionsAndReasons with why, activeWork including explored-but-unconfirmed directions and rejected alternatives, unresolvedQuestions, referencedObjects, nextTurnAnchor) under contract `morpho-agent-compaction-v2-2026-08-16`; the summary schema is unchanged and summaries remain non-authoritative. (Superseded on 2026-08-17 by compaction v3 — rejected/eliminated alternatives move to decisionsAndReasons.)

Image compiler: compilation now asserts design intent over rendering decoration and grounds scenario visuals in scale, action, and human-product relationship. The structured intent to deterministic compiler pipeline is unchanged. (Contract bumped to `morpho-image-prompt-v3` on 2026-08-17 to keep version-as-content-identity.)
...
## 2026-08-17: A+ Closure Pass — Web Search Authority, Compare Authority, Clause-First Memory, Final Check

Decision: close the cross-layer correctness gaps found in the pre-acceptance review without expanding the capability layer.

- Web Search authority preservation: `AgentTurnCoordinator.copyProviderRequest()` dropped `capabilityIntent.webSearch`, so an authorized turn lost the bit at the Coordinator boundary and the server normalized it to false. The copy now preserves the optional bit and `isProviderRequestShape` validates it fail-closed (`undefined | boolean` only). The bit now survives initial request, active-request copy, recovery export/restore, exact retry, and continuation; the server contract (global web-search enabled AND current-turn user authority) is unchanged and no other input can expand it.
- Compare authority split: ordinary comparison ("把这两个比较一下") is chat-only by default; a persisted Compare record requires an explicit save intent BOUND to the Compare record ("保留比较记录" / "保存这次比较" / "创建比较记录" / "记录一下比较结果" / "把比较结果留在项目里" / "把比较结论存档"). Implemented as `isExplicitComparisonRecordRequest` (explicit Compare-owned noun persistence — verb→compare noun or 把…比较…留下/存档 — plus an immediate-result binding that is a BARE-result positive proof, structural rather than blacklist-based: only a structurally bare 结果/结论 directly bound to the compare persist action is Compare-owned, i.e. verb-then-result with a suffix-only gap ("比较一下，记录一下结果"), a disposal marker 把/将 sitting directly before the result word ("对比这两个方案，把结论存档"), or the explicit Compare compound noun ("比较结果保存"); any lexical modifier between the disposal context and the result word ("把研究的结论存档" / "把研究最终结论存档" / "把研究所得结论存档" / "把测试最终结果保存") disables the inference, and explicit Compare-owned forms like "把这次比较的结论存档" remain authorized) gating both the local authority profile (`allowComparisonWrite`) and the executor (`getComparisonToolExecutionBlockReason`), so a model calling `create_comparison_analysis` on a plain comparison is blocked locally before any Workspace write. Ownership is explicit: "比较这两个方案，把这个研究结论保存一下" / "比较两个方案，然后记录一下测试结果" / "比较两个方案，把研究的结论存档" stay closed — a foreign-domain result never attaches to the Compare record. Negation is decoupled at clause level: `isExplicitComparisonRequest` is a clause-level resolver where only a clause negating the compare action itself ("不要比较，只分析") closes the comparison, while "比较一下，但不要保存记录" keeps comparison on and persist off (and still loads `comparisonDecision`). The adverb usage "比较省钱" is shared-scrubbed between both detectors and the strategy resolver. The canonical comparison strategy text and the tool description state the chat-only default. Because the canonical strategy text changed, the Agent Prompt Contract bumps to `morpho-agent-v3.6-2026-08-17`. A+ execution integration: the A+ preparation no longer crashes on comparison turns — comparison task contexts are routed through the new `buildProviderComparisonTaskContext` (the previously orphaned `buildProviderComparisonBackgroundContext` plus selected-object lists and document extracts) instead of the generic `buildProviderTaskContext`, whose comparison guard stays as misuse protection; the comparison-only scope (explicit selected objects, no default-reference auto-inclusion) still rides in the task context and the trusted turn frame.
- Clause-first memory semantics: the guard no longer early-returns a whole message on one-off markers and no longer uses bare 统一/稳定 as long-term scope. Every clause is admitted independently (operation boundary → agent/tool operational-instruction gate → one-off scope guard with explicit project/long-term/quantitative-subject overrides → declaration classifier), and kind classification is declaration-based rather than keyword-based: the resolver answers "is the user explicitly stating information that should stay true". Current-turn agent/tool operation commands — a behavior directive (不要/别/无需/只/仅/先/就/直接/再) paired with an agent action verb (比较/对比/联网/搜索/保存/存档/写入/创建/生成/总结/分析/修改/删除/更新/执行/记录…) — are not project declarations ("不要保存记录" / "不要比较" / "不要联网" / "不要创建研究分析" / "不要生成图片" / "必须先联网查一下" / "先生成两张" / "只总结不要写入" produce no candidate), unless an explicit long-term or project scope PRECEDES the operation ("以后这个项目都不要联网" / "整个项目不要自动保存比较记录" stay legitimate cross-turn behavior preferences); a scope noun after the verb is the operation's object, not a scope ("不要保存项目"). Preference stance verbs (使用/采用/用/保持/沿用), the memory-intent verb 记住, and normative constraint markers (必须/不能/不得) are outside the gate, so "不使用镜面金属" / "不要高反光" / "尺寸必须小于 200mm" / "记住这个尺寸" keep classifying as avoidance/constraint. A scope word alone is not a preference ("后续怎么做？" / "长期来看还有什么风险？"); questions and threshold inquiries are not declarations ("这个材质怎么样？" / "高度低于多少合适？" / "有哪些待确认问题？" / "是不是不要高反光？"); temporary instructions are not avoidance ("先别用蓝色" / "暂时不要蓝色" — TEMPORARY_SCOPE_PATTERN alongside the one-off scope); structured-state commands ("默认参考改成这张") are handled by real workspace state; preference requires an explicit stance (我喜欢/我偏好/默认用/希望保持/以后都用/统一采用…), constraint requires normative syntax (必须/不得/不能/不超过/上限/控制在/限制在…), openQuestion requires a declarative unresolved tail (X 还需确认 / X 尚不确定) while queries about existing unresolved items are rejected, and "是否支持单手操作还需确认" remains a legal declaration. Mixed messages keep their legal clauses ("这次先把背景换白色；预算不能超过 500 元" retains only the budget constraint). The resolver is only an authority precondition; it never writes memory itself.
- Deterministic memory final check: when candidates exist, the initial Provider input carries ONE transient runtime-control reminder (never persisted to `workspace.ai.messages`) requiring `submit_memory_update` with verbatim evidence or `items: []` + non-empty `skippedReason` before the turn ends. `memoryUpdateReminderInserted` enters Recovery facts, the reminder rides the exact request body (retry/refresh/recovery verbatim), and the flag is set once at preparation, so no loop and no regeneration. A post-hoc continuation after a no-Tool final answer is impossible under the A+ Journal contract (no-Tool answers settle terminal `externallyCompleted`; continuations require `awaitingNextRequest` plus non-empty Tool items), so the reminder is proactive — this boundary is documented, not worked around.
- Compaction v3: confirmed rejected/eliminated alternatives move from `activeWork` to `decisionsAndReasons` (with their reasons); `activeWork` keeps only explored-but-unconfirmed candidates and current explorations. Contract `morpho-agent-compaction-v3-2026-08-17`.
- Image prompt contract: content identity bumped to `morpho-image-prompt-v3`; Generation Record schema unchanged.
- Concept refinement boundary: 优化/改进 on a direction ("优化这个方向", "改进一下方案 A", "把这个方案优化一下") resolve to `conceptRefinement`; fresh-direction requests ("再给我三个新方向", "重新想几个完全不同的方向", "基于这个做一个全新的方向") stay divergence.

Boundary: unchanged from the 2026-08-16 ADR; no runtime rewrite, no new server Journal protocol or database schema, no multi-version prompt compatibility layer (rollout boundary recorded in `docs/operations/runbook.md`). G1 (exact budget / item-pressure production wiring), frame timeline filter production use, runtimeConfiguration frame creation, and cache-hint key completeness remain recorded debt, not fixed here. The comparison preparation crash discovered during this closure (`buildProviderTaskContext` rejecting the comparison context kind on the A+ path, main-chain `1ea74b9`) was an integration gap in the Compare chain itself and is fixed above, not deferred.
