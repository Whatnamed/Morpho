...
- Every completed non-summary Provider request signs an output-inclusive durable transcript checkpoint and persists it only in `workspace.ai.latestProviderRequestState`; historical traces strip both its manifest hash and token. The 24-hour ordinary-use window is renewable through a longer signed refresh lifetime at `/api/ai/agent/snapshot/refresh`, which performs no Provider or Lease work. Compatible legacy text manifests can acquire message IDs during renewal; changed content, existing IDs, project scope or signatures cannot.
- Durable user messages replace image Data URLs with server-verified object ID, asset ID, content hash and MIME references; the server returns those verified references for persistence. Durable Assistant messages use an immutable raw Provider-output snapshot independently from display sanitization. Transcript manifest v3 binds message and strategy items to workspace message IDs.
...
- A Summary may carry only Context markers authorized by the prior continuation/snapshot or newly bound to the complete current Call batch and terminal outputs. Receipt v4 binds their ordered manifest, including placement, sequence, anchor and causal binding. Baseline markers remain before the summary; after-message and tool-causal markers remain after the retained tail and its terminal outputs. Fresh current-turn frames remain receipt-bound data, and a completed post-compaction request includes those retained frame identities in its closed snapshot.
