...
Boundary: unchanged from the 2026-08-16 ADR; no runtime rewrite, no new server Journal protocol or database schema, no multi-version prompt compatibility layer (rollout boundary recorded in `docs/operations/runbook.md`). G1 (exact budget / item-pressure production wiring), frame timeline filter production use, runtimeConfiguration frame creation, and cache-hint key completeness remain recorded debt, not fixed here. Additionally recorded during this closure: an explicit comparison turn fails at client preparation because `buildProviderTaskContext` rejects the `comparison` context kind (main-chain `1ea74b9`, present in `origin/main` before this branch) — recorded, not fixed, and it blocks only the runner-level e2e row for "比较一下，但不要保存记录", whose compare-side authorities are locked at the strategy/authority test layer and whose memory side is proven through an identical-clause discussion draft.
