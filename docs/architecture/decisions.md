...
- Compare authority split: ordinary comparison ("把这两个比较一下") is chat-only by default; a persisted Compare record requires an explicit save intent BOUND to the Compare record ("保留比较记录" / "保存这次比较" / "创建比较记录" / "记录一下比较结果" / "把比较结果留在项目里" / "把比较结论存档"). Implemented as `isExplicitComparisonRecordRequest` (bound save-action patterns: verb→compare, 把…比较…留下/存档, result/结论 bound to a save verb; negation-aware; adverb usage "比较省钱" shared-scrubbed with `isExplicitComparisonRequest`, which the strategy resolver also uses) gating both the local authority profile (`allowComparisonWrite`) and the executor (`getComparisonToolExecutionBlockReason`), so a model calling `create_comparison_analysis` on a plain comparison is blocked locally before any Workspace write. Proximity alone never grants: "比较结果怎么样？" / "创建两个方案然后比较一下" / "记录一下预算，再比较两个方案" stay closed. The canonical comparison strategy text and the tool description state the chat-only default. Because the canonical strategy text changed, the Agent Prompt Contract bumps to `morpho-agent-v3.6-2026-08-17`.
- Clause-first memory semantics: the guard no longer early-returns a whole message on one-off markers and no longer uses bare 统一/稳定 as long-term scope. Every clause is admitted independently (operation boundary → one-off scope guard with explicit project/long-term/quantitative-subject overrides → kind classifier). Stable preference intent ("我喜欢低饱和配色", "默认用暖灰色", "我偏好哑光材质", "以后所有方向都不要高反光") is now recalled without any scope word, while "这张图统一一下配色"/"这版保持哑光" stay rejected. Kind classification additionally requires high-confidence stance: constraint needs normative syntax (必须/不得/不能/不超过/至少/上限/控制在/限制在…) and preference needs an explicit stance (我喜欢/我偏好/默认/以后/统一采用/希望保持…), so ordinary design discussion ("这个材质怎么样？", "这个尺寸合适吗？", "颜色换哪个更好？") never produces a candidate or memory authority. Mixed messages keep their legal clauses ("这次先把背景换白色；预算不能超过 500 元" retains only the budget constraint).
