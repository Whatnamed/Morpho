...
- Compare authority split: ordinary comparison ("把这两个比较一下") is chat-only by default; a persisted Compare record requires an explicit save intent ("保留比较记录" / "保存这次比较" / "创建比较记录" / "把比较结果留在项目里"). Implemented as `isExplicitComparisonRecordRequest` (negation-aware, adjective-usage safe) gating both the local authority profile (`allowComparisonWrite`) and the executor (`getComparisonToolExecutionBlockReason`), so a model calling `create_comparison_analysis` on a plain comparison is blocked locally before any Workspace write. The canonical comparison strategy text and the tool description state the chat-only default. Because the canonical strategy text changed, the Agent Prompt Contract bumps to `morpho-agent-v3.6-2026-08-17`.
- Clause-first memory semantics: the guard no longer early-returns a whole message on one-off markers and no longer uses bare 统一/稳定 as long-term scope. Every clause is admitted independently (operation boundary → one-off scope guard with explicit project/long-term/quantitative-subject overrides → kind classifier). Stable preference intent ("我喜欢低饱和配色", "默认用暖灰色", "我偏好哑光材质", "以后所有方向都不要高反光") is now recalled without any scope word, while "这张图统一一下配色"/"这版保持哑光" stay rejected. Mixed messages keep their legal clauses ("这次先把背景换白色；预算不能超过 500 元" retains only the budget constraint).
