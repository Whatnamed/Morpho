...
- `src/server/ai/providerResponseBoundary.ts` bounds buffered JSON to 8 MiB, diagnostic reads to 16 KiB/800 exposed characters, total SSE transport to 32 MiB, and an unfinished SSE frame to 1 MiB. One 15-minute safety budget spans fetch, retry waits, prompt-cache compatibility retry, stream parsing, and buffered fallback. Explicit cancellation remains `AbortError`; an internal deadline is a stable Provider failure, and either transport-limit failure is terminal rather than a reason for a second buffered or image-stripping request;
...
- `submit_memory_update` accepts only locally validated candidates backed by an exact quote from the persisted current user message. Deterministic projections remain authoritative; AI suggestions and one-off generation requests do not become user preferences. Admission is clause-first: clauses addressing only this turn's concrete image/object/version (e.g. "这张图不要高反光", "这张图统一一下配色") are rejected as one-off unless an explicit project/long-term scope or a quantitative constraint SUBJECT overrides them; stable preference intent ("我喜欢低饱和配色", "默认用暖灰色") is recalled without any scope word, and kind classification requires high-confidence stance (normative syntax for constraint, an explicit 我喜欢/我偏好/默认/以后/统一采用… stance for preference), so ordinary design discussion ("这个材质怎么样？", "这个尺寸合适吗？") never produces a candidate. When candidates exist, the initial Provider input carries ONE transient runtime-control reminder (never persisted to the conversation) requiring `submit_memory_update` with verbatim evidence or `items: []` + non-empty `skippedReason` before the turn ends; the armed flag lives in Recovery facts so refresh/retry never re-arms or regenerates it.
