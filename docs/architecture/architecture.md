...
- every completed non-summary Provider response replaces `workspace.ai.latestProviderRequestState` with one output-inclusive, 24-hour signed transcript snapshot. Its durable manifest contains canonical conversation messages and strategy items, excludes request-local reasoning/Call/terminal-output items, and is stripped from historical Agent traces so the next cross-turn compaction has one unambiguous proof source;
...
- compaction carries only the effective Context Frame timeline: the active post-summary turn frames plus the latest project/runtime state. Typed markers must be authorized by the prior continuation/snapshot claim or by the current complete Call/terminal-output causal binding; a current unbound frame remains receipt-bound data until a completed request closes the snapshot;
...
- Provider transcript continuation uses a v3 HMAC token. Its exact tail admits only the signed Provider output, one terminal output per signed function call, and server-authorized typed Context/State markers; tool-created markers additionally bind the signed Call batch and terminal outputs. Ordinary messages, new Calls, strategy markers, unknown Items, and missing terminal outputs fail before the Lease RPC. The token contains no transcript body and expires after 20 minutes.
