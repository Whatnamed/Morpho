...
- Provider transcript continuation uses a v2 HMAC token. Its exact tail admits only the signed Provider output, one terminal output per signed function call, and server-verified typed Context/State markers; ordinary messages, new Calls, strategy markers, unknown Items, and missing terminal outputs fail before the Lease RPC. The token contains no transcript body and expires after 20 minutes.
- Conversation compaction carries a bounded source/tail `CompactionDescriptor`. A successful server-normalized summary produces a signed receipt containing summary and tail hashes, revision/boundary metadata, Lease/Turn/Sequence scope, Prompt Contract, and expiry. The receipt is kept in the transient token only; raw messages remain local and post-compaction replay re-hashes the strict typed marker and retained tail before counting the Lease.
