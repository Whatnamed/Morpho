...
- every completed non-summary Provider response replaces `workspace.ai.latestProviderRequestState` with one output-inclusive signed transcript checkpoint. Its ordinary-use window is 24 hours, while `/api/ai/agent/snapshot/refresh` can verify and renew the project-scoped checkpoint without Provider or Lease work during the longer refresh window. Its durable manifest binds workspace message IDs, canonical user/assistant facts and strategy items, excludes request-local reasoning/Call/terminal-output items and image Base64, and is stripped from historical Agent traces;
...
- compaction carries only the effective Context Frame timeline: the active post-summary turn frames plus the latest project/runtime state. The receipt binds an ordered marker manifest; baseline markers precede the summary/tail, while after-message and tool-causal markers follow the retained terminal outputs. Typed markers must be authorized by the prior continuation/snapshot claim or by the current complete Call/terminal-output causal binding;
...
- Provider transcript continuation uses a v4 HMAC token. Its exact tail admits only the signed Provider output, one terminal output per signed function call, and server-authorized typed Context/State markers; tool-created markers additionally bind the signed Call batch and terminal outputs. Ordinary messages, new Calls, strategy markers, unknown Items, and missing terminal outputs fail before the Lease RPC. The token contains no transcript body and expires after 20 minutes.
- Conversation compaction carries a bounded source/tail `CompactionDescriptor` whose protocol digests are domain-separated SHA-256. A successful server-normalized summary produces a signed receipt containing summary and tail hashes, revision/boundary metadata, Lease/Turn/Sequence scope, Prompt Contract, and expiry. The receipt is kept in the transient token only; raw messages remain local and post-compaction replay re-hashes the strict typed marker and retained tail before counting the Lease.
