...
name: neat-freak
description: >
  Reconcile Morpho project documentation and agent instructions with the current code
  after a meaningful implementation milestone, architecture decision, integration change,
  or explicit handoff request. Trigger only when the user explicitly says:
  "/neat", "同步 Morpho 文档", "对齐项目文档", "做一次完整交接",
  "整理整个项目的开发文档", or asks to reconcile docs and code.
  Do not trigger for ordinary UI cleanup, copy editing, local refactoring,
  small CSS changes, or generic phrases such as "整理一下" and "收尾".
---
