...
`MORPHO_MIMO_API_KEYS` 是推荐的多 key 配置，并优先于旧的单 key 写法。兼容优先级为：`MORPHO_MIMO_API_KEYS` -> `MORPHO_MIMO_API_KEY` + `MORPHO_MIMO_API_KEY_2` -> `MORPHO_MIMO_API_KEY`；其中 `MORPHO_MIMO_API_KEY_2` 只作为备用 key，不单独作为主 key。多模态模型优先级为 `MORPHO_MIMO_MULTIMODAL_MODEL` -> `MORPHO_MIMO_MODEL` -> `mimo-v2.5`。GrsAI 默认模型优先级为 `MORPHO_GRS_DEFAULT_MODEL` -> `MORPHO_GRS_IMAGE_MODEL`。新环境应迁移到 plural key、`MORPHO_MIMO_MULTIMODAL_MODEL` 和 `MORPHO_GRS_DEFAULT_MODEL`。
