/**
 * Synchronous SHA-256 for protocol values shared by the browser and Node
 * runtimes. Keeping this small implementation local avoids making the client
 * bundle depend on `node:crypto` or an additional hashing package.
 */
...
export function sha256Hex(value: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      schedule[index] = (
        smallSigma1(upper) + schedule[index - 7]! + smallSigma0(lower) + schedule[index - 16]!
      ) >>> 0;
    }
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + bigSigma1(e) + choose + ROUND_CONSTANTS[index]! + schedule[index]!) >>> 0;
      const temp2 = (bigSigma0(a) + majority) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + temp1) >>> 0;
