...
export async function runMorphoAgentTurnAPlus(
  ...
  const recovered = await recoverMorphoAgentTurnAPlus(localProjectId, host, dependencies);
  ...
  try {
    ...
    if (lifecycle?.phase === "terminal" || prepared.controller.signal.aborted) {
      activeSessions.delete(localProjectId);
    }
  }
}

export async function recoverMorphoAgentTurnAPlus(
  localProjectId: string,
  host: AgentTurnHost,
  dependencies: AgentTurnRunnerAPlusDependencies = {}
...
export function detachMorphoAgentTurnAPlusForPageUnload(localProjectId: string): void {
  activeSessions.delete(localProjectId);
}

export async function acknowledgeMorphoAgentPendingConfirmationAPlus(
  localProjectId: string,
...
export async function runManualCompactionTurnAPlus(
  input: RunMorphoAgentTurnAPlusInput,
  host: AgentTurnHost,
  dependencies: AgentTurnRunnerAPlusDependencies = {}
): Promise<void> {
